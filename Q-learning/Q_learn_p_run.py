from Q_learning_with_p import Q_learn_main_with_p
import os
import neptune
import numpy as np
...
if __name__ == "__main__":
    ...
    for grid_size, number_of_actions in param_to_try:
        for _ in range(5):
            ...
            t, it_no, run_id = Q_learn_main_with_p(params_to_try)
        ...
        run[f"time/time_Q_p_grid_{grid_size}_nac_{number_of_actions}"]\
            = str(param_to_try[(grid_size, number_of_actions)]["time"])
        run[f"it_number/it_no_Q_p_grid_{grid_size}_nac_{number_of_actions}"]\
            = str(param_to_try[(grid_size, number_of_actions)]["it_number"])
        run[f"run_id/run_Q_p_grid_{grid_size}_nac_{number_of_actions}"]\
